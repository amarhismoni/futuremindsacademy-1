...
function getWinner(game) {
    ...
    //check rows
    for (let row = 0; row < 3; row++) {
        if (game[row][0] !== "." && allEqual(game[row])) return game[row][0];
    }
    ...
    for (let col = 0; col < 3; col++) {
        ...
        if (game[0][col] != "." && allEqual(colArray)) return game[0][col];
    }
    ...
    if (game[0][0] !== "." && allEqual(firstDiagonalArray)) return game[0][0];

    if (game[0][2] !== "." && allEqual(secondDiagonalArray)) return game[0][2];
...
//implement fs.watch instead of while true do 